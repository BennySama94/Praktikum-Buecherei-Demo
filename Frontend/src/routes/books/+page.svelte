<script lang="ts">
    ...
    async function loadBooks(): Promise<void> {
        loading = true;
        try {
            const res = await api.get<BooksResponse>('/books'); // ← was /v1/books
            books = res.data;
        } catch {
    ...
    async function borrow(bookId: number): Promise<void> {
        try {
            await api.post('/loans', { book_id: bookId }); // ← was /v1/loans
            await loadBooks();
        } catch (err: unknown) {
    ...
    async function deleteBook(bookId: number): Promise<void> {
        ...
        try {
            await api.delete(`/books/${bookId}`); // ← was /v1/books/{id}
    ...
    async function createBook(): Promise<void> {
        ...
        formLoading = true;
        try {
            await api.post('/books', { ...form }); // ← was /v1/books
            showForm = false;
...
<div class="min-h-screen bg-gray-100 p-8">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded shadow p-6 mb-6">
            ...
            {#if formError}<p class="text-red-500 text-sm mb-3">{formError}</p>{/if}
            <form on:submit|preventDefault={createBook} class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="author" bind:value={form.author} required
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1" for="isbn">ISBN</label>
                    <input id="isbn" bind:value={form.isbn} required
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1" for="genre">Genre</label>
                    <input id="genre" bind:value={form.genre} required
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1" for="year">Jahr</label>
                    <input id="year" type="number" bind:value={form.year} required
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1" for="copies">Exemplare</label>
                    <input id="copies" type="number" min="1" bind:value={form.total_copies} required
                           class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                ...
                <div class="bg-white rounded shadow p-4 flex justify-between items-start">
                    ...
                    <div class="flex flex-col items-end gap-2 ml-4">
                        ...
                        {#if user?.role === 'member' && book.available_copies > 0}
                            <button on:click={() => borrow(book.id)}
